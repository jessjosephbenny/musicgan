...
class Model(object):
    ...
    def init_all(self):
        ...
        tf.compat.v1.variables_initializer(tf.compat.v1.global_variables(self.scope.name)).run()
    ...
    def get_adversarial_loss(self, discriminator, scope_to_reuse=None):
        ...
        if self.config['gan']['type'] == 'gan':
            ...
            adv_loss_g = tf.compat.v1.losses.sigmoid_cross_entropy(
                tf.ones_like(self.D_fake.tensor_out), self.D_fake.tensor_out)

        if (self.config['gan']['type'] == 'wgan'
                or self.config['gan']['type'] == 'wgan-gp'):
            adv_loss_d = (tf.reduce_mean(input_tensor=self.D_fake.tensor_out)
                          - tf.reduce_mean(input_tensor=self.D_real.tensor_out))
            adv_loss_g = -tf.reduce_mean(input_tensor=self.D_fake.tensor_out)

            if self.config['gan']['type'] == 'wgan-gp':
                eps = tf.random.uniform(
                    [tf.shape(input=self.x_)[0], 1, 1, 1, 1], 0.0, 1.0)
                inter = eps * self.x_ + (1. - eps) * self.G.tensor_out
                if scope_to_reuse is None:
                    D_inter = discriminator(inter, self.config, name='D',
                                            reuse=True)
                else:
                    with tf.compat.v1.variable_scope(scope_to_reuse, reuse=True):
                        D_inter = discriminator(inter, self.config, name='D',
                                                reuse=True)
                gradient = tf.gradients(ys=D_inter.tensor_out, xs=inter)[0]
                slopes = tf.sqrt(1e-8 + tf.reduce_sum(
                    input_tensor=tf.square(gradient),
                    axis=tf.range(1, len(gradient.get_shape()))))
    ...
    def get_statistics(self):
        ...
        def get_num_parameter(var_list):
            ...
            return int(np.sum([np.product([x.value for x in var.get_shape()])
                               for var in var_list]))
        num_par = get_num_parameter(tf.compat.v1.trainable_variables(
            self.scope.name))
    ...
    def get_global_step_str(self):
        ...
        return str(tf.compat.v1.train.global_step(self.sess, self.global_step))
