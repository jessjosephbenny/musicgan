...
class Layer(object):
    ...
    def build(self, structure, condition, slope_tensor):
        ...
        if self.layer_type == 'mean':
            ...
            return tf.reduce_mean(input_tensor=self.tensor_in, axis=structure[1], keepdims=keepdims,
                                  name='mean')

        # Summation layers
        if self.layer_type == 'sum':
            keepdims = structure[2] if len(structure) > 2 else None
            return tf.reduce_sum(input_tensor=self.tensor_in, axis=structure[1], keepdims=keepdims,
                                 name='sum')
        ...
        if self.layer_type == 'avgpool2d':
            return tf.compat.v1.layers.average_pooling2d(self.tensor_in, structure[1][0],
                                               structure[1][1],
                                               name='avgpool2d')
        if self.layer_type == 'maxpool2d':
            return tf.compat.v1.layers.max_pooling2d(self.tensor_in, structure[1][0],
                                           structure[1][1], name='maxpool2d')
        if self.layer_type == 'avgpool3d':
            return tf.compat.v1.layers.average_pooling3d(self.tensor_in, structure[1][0],
                                               structure[1][1],
                                               name='avgpool3d')
        if self.layer_type == 'maxpool3d':
            return tf.compat.v1.layers.max_pooling3d(self.tensor_in, structure[1][0],
        ...
        if self.layer_type == 'dense':
            ...
            self.core = tf.compat.v1.layers.dense(self.conditioned, structure[1],
            ...
            if self.layer_type == 'conv1d':
                self.core = tf.compat.v1.layers.conv1d(
        ...
        if normalization is None:
            ...
            self.normalized = tf.compat.v1.layers.batch_normalization(
