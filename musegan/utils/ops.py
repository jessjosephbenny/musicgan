...
def bernoulli_sample(x):
    ...
    with ops.name_scope("BernoulliSample") as name:
        with g.gradient_override_map({"Ceil": "Identity",
            ...
            return tf.math.ceil(x - tf.random.uniform(tf.shape(input=x)), name=name)

@ops.RegisterGradient("BernoulliSample_ST")
def bernoulli_sample_ST(op, grad):
    return [grad, tf.zeros(tf.shape(input=op.inputs[1]))]
...
def binary_stochastic_REINFORCE(x, loss_op_name="loss_by_example"):
    ...
    with ops.name_scope("BinaryStochasticREINFORCE"):
        with g.gradient_override_map({"Sigmoid": "BinaryStochastic_REINFORCE",
            ...
            return tf.math.ceil(p - tf.random.uniform(tf.shape(input=x)))
...
def _binaryStochastic_REINFORCE(op, _):
    ...
    outcome_diff_sq_r = tf.reduce_mean(input_tensor=outcome_diff_sq, axis=0)
    outcome_diff_sq_loss_r = tf.reduce_mean(
        input_tensor=outcome_diff_sq * tf.expand_dims(loss_tensor, 1), axis=0)
    ...
    decay = 0.95
    train_l_bar_num = tf.compat.v1.assign(l_bar_num, l_bar_num*decay +\
                                            outcome_diff_sq_loss_r*(1-decay))
    train_l_bar_den = tf.compat.v1.assign(l_bar_den, l_bar_den*decay +\
...
def binary_wrapper(pre_activations_tensor, estimator,
    ...
    *slope_tensor: for ST only - tensor specifying the slope for purposes of
    ...
    if estimator == 'straight_through':
        if pass_through:
            return tf.cond(
                pred=stochastic_tensor,
                true_fn=lambda: binary_stochastic_ST(pre_activations_tensor),
                false_fn=lambda: binary_stochastic_ST(pre_activations_tensor,
                                             stochastic=False))
        else:
            return tf.cond(
                pred=stochastic_tensor,
        ...
        return tf.cond(
            pred=stochastic_tensor,
            true_fn=lambda: binary_stochastic_REINFORCE(pre_activations_tensor),
            false_fn=lambda: binary_stochastic_ST(pre_activations_tensor,
                                         stochastic=False))
